...
// SitePageAudit is one auxiliary page audited during a deep scan.
type SitePageAudit struct {
	URL      string `json:"url"`
	Title    string `json:"title,omitempty"`
	SEOScore int    `json:"seoScore"` // 0–100 required-check pass rate
	Status   string `json:"status"`   // "ok" | "error"
	Error    string `json:"error,omitempty"`
}

// SitePagesAudit is the per-page rollup attached to deep-scan results.
type SitePagesAudit struct {
	Pages       []SitePageAudit `json:"pages"`
	AvgSEOScore int             `json:"avgSeoScore"` // includes the main page
...
type AnalysisResult struct {
	...
	SitePages          *SitePagesAudit       `json:"sitePages,omitempty"`
	Usage              *UsageSummary         `json:"usage,omitempty"`
	// AISummary is a 3-paragraph plain-English narrative generated by the LLM.
	// Empty when the Groq integration is disabled (no GROQ_API_KEY) or the call
	// failed — UI should hide the section in that case.
...
type AnalyzeRequest struct {
	URL string `json:"url"`
	// Refresh skips the short-lived result cache so "re-run fresh" always
	// performs a new fetch+analysis.
	Refresh bool `json:"refresh,omitempty"`
	// Deep additionally audits a few key subpages (/pricing, /about, …) and
	// attaches a per-page rollup to the result.

...
export function AccountDashboard({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/75 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="w-full max-w-2xl max-h-[88vh] overflow-y-auto rounded-xl border border-zinc-800 bg-zinc-900 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between gap-4 border-b border-zinc-800 px-5 py-4">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-wider text-violet-400">Account</p>
            <h2 className="mt-1 text-base font-semibold text-zinc-100">Usage & integrations</h2>
          </div>
        ...
        <div className="flex gap-1 border-b border-zinc-800 px-4 pt-3">
          {(["usage", "keys", "webhooks"] as AccountTab[]).map((item) => (
            <button key={item} onClick={() => setTab(item)} className={`rounded-t-md px-3 py-2 text-xs font-semibold capitalize transition-colors ${tab === item ? "border-b-2 border-violet-400 text-violet-300" : "text-zinc-500 hover:text-zinc-300"}`}>
        ...
        <div className="p-5">
          ...
          {!loading && tab === "keys" && (
            <div className="space-y-4">
              ...
              <div className="flex gap-2"><input value={keyName} onChange={(e) => setKeyName(e.target.value)} maxLength={64} placeholder="Key name, e.g. CI audit bot" className="min-w-0 flex-1 rounded-md border border-zinc-800 bg-zinc-950 px-3 py-2 text-xs text-zinc-200 outline-none focus:border-violet-500/50" /><button onClick={() => void handleCreateKey()} disabled={busy === "create-key"} className="rounded-md bg-violet-500 px-3 py-2 text-xs font-semibold text-white hover:bg-violet-400 disabled:opacity-50">{busy === "create-key" ? "Creating…" : "Create key"}</button></div>
          ...
          {!loading && tab === "webhooks" && (
            <div className="space-y-4">
              ...
              <div className="flex gap-2"><input value={webhookURL} onChange={(e) => setWebhookURL(e.target.value)} placeholder="https://example.com/webhooks/website" className="min-w-0 flex-1 rounded-md border border-zinc-800 bg-zinc-950 px-3 py-2 text-xs text-zinc-200 outline-none focus:border-violet-500/50" /><button onClick={() => void handleCreateWebhook()} disabled={busy === "create-webhook"} className="rounded-md bg-violet-500 px-3 py-2 text-xs font-semibold text-white hover:bg-violet-400 disabled:opacity-50">{busy === "create-webhook" ? "Adding…" : "Add webhook"}</button></div>
